...
static inline void DrawPoint(int x, int y)
{
    ...
    if (y < 8) {
        UI_DrawPixelBuffer(&gStatusLine, (uint8_t)x, (uint8_t)y, true);
        return;
    }
    UI_DrawPixelBuffer(gFrameBuffer, (uint8_t)x, (uint8_t)(y - 8), true);
}

static inline void UART_READ(void)
{
#ifdef ENABLE_UART
...
static bool TLE_KeyEventShouldFire(KEY_Code_t key)
{
    // KEYBOARD_Poll() returns the currently pressed key. Without edge detection,
    // a single press is seen repeatedly (every loop tick), making controls feel
    // overly sensitive.
    //
    // Policy:
    // - Most keys: fire once per physical press.
    // - UP/DOWN: allow slow auto-repeat while held.
    static KEY_Code_t lastKey = KEY_INVALID;
    static uint32_t keyDownSinceMs = 0U;
    static uint32_t lastRepeatMs = 0U;
    ...
    // Key held: allow limited repeat for navigation keys.
    if (key == KEY_UP || key == KEY_DOWN) {
        const uint32_t heldMs = (uint32_t)(now - keyDownSinceMs);
        const uint32_t sinceRepeatMs = (uint32_t)(now - lastRepeatMs);
        if (heldMs >= 450U && sinceRepeatMs >= 160U) {
            lastRepeatMs = now;
            return true;
        }
    }
...
static inline uint8_t TLE_DaysInMonth(uint16_t year, uint8_t month)
{
    static const uint8_t mdays[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    uint8_t d = (month >= 1 && month <= 12) ? mdays[month - 1] : 31;
...
static void TLE_AddHours(uint8_t t[6], int hours)
{
    // t: [YY,MM,DD,hh,mm,ss] where YY is 2000-based.
    if (!t) {
        return;
    }
    int h = (int)t[3] + hours;
    ...
    int yy = (int)t[0];
    int mm = (int)t[1];
    int dd = (int)t[2] + day_delta;

    while (dd > 0) {
        const uint16_t year = (uint16_t)(2000 + yy);
        const uint8_t dim = TLE_DaysInMonth(year, (uint8_t)mm);
        if (dd <= (int)dim) {
            break;
        }
        dd -= (int)dim;
...
static inline void utcToBeijingTime(uint8_t t[6])
{
    TLE_AddHours(t, 8);
}

static inline void beijingToUtcTime(uint8_t t[6])
{
    TLE_AddHours(t, -8);
}

static void TLE_SystemReset(void)
{
#if defined(ARDUINO_ARCH_ESP32)
    esp_restart();
#else
...
static const uint8_t BITMAP_ARRAY_EMPTY_RIGHT[7] =
{
    //                0b00000000,
    0b01111111,
    0b01000001,
...
static const uint8_t BITMAP_ARRAY_RIGHT[7] =
{
    //                0b00000000,
    0b01111111,
    0b01111111,
...
static const uint8_t BITMAP_ARRAY_LEFT[7] =
{
    //                0b00000000,
    0b00001000,
    0b00011100,
...
uint8_t cal_sat(char line0[25], char line1[70], char line2[70], lat_lon observer, look_result *look, lat_lon *sub_point,
                uint8_t my_time[6]) {
    ...
    *sub_point = eci_to_lat_lon(&data, target);
    sub_point->distance = sqrt(
        (look->observer_eci.x - sub_point->sate_eci.x) * (look->observer_eci.x - sub_point->sate_eci.x)
...
void Read_TLE(uint8_t num, sat_parm *now_sat) {
    ...
    now_sat->TX_FREQ = c[9 + 69 + 69 + 4] + (c[9 + 69 + 69 + 5] << 8) + (c[9 + 69 + 69 + 6] << 16) + (
                           c[9 + 69 + 69 + 7] << 24); // 上行发射频率/10 uint32，低位在前高位在后
    now_sat->RX_FREQ = c[9 + 69 + 69 + 8] + (c[9 + 69 + 69 + 9] << 8) + (c[9 + 69 + 69 + 10] << 16) + (
                           c[9 + 69 + 69 + 11] << 24); // 下行接收频率/10 uint32，低位在前高位在后
    now_sat->line2[69] = 0;
    now_sat->line1[69] = 0;
    now_sat->name[9] = 0;
}
...
uint8_t tle_check() {
    ...
    for (int i = 0; i < 45; i++) {
        ...
        if (now_sat.TX_TONE > 2541 || now_sat.RX_TONE > 2541 || now_sat.RX_FREQ == 0 || now_sat.TX_FREQ > 50000000 ||
            now_sat.RX_FREQ > 50000000)
            judge = 0;
        if (judge) {
...
void SELECT_DISPLAY() {
    ...
    if ((now_menu - 1) / NUM_PER_PAGE * NUM_PER_PAGE + 1 < sate_info.num)
        memcpy(
            &gStatusLine[110], BITMAP_ARRAY_DOWN, 5);
    if ((now_menu - 1) / NUM_PER_PAGE) memcpy(&gStatusLine[102], BITMAP_ARRAY_UP, 5);
#ifdef ENABLE_RTC
    UI_PrintStringSmallBuffer("LoseHu DSF 0.2R", gStatusLine);
#else
    UI_PrintStringSmallBuffer("LoseHu DSF 0.2", gStatusLine);
#endif
...
void SATE_DISPLAY() {
    ...
    // my_time is stored as Beijing local time (from PCF8563 / k5web sync).
    // Convert to UTC only when requested for display.
    if (!ENABLE_BTC) {
        beijingToUtcTime(SHOW_TIME);
    }

    sprintf(String, "20%02d-%02d-%02d %02d:%02d:%02d ", SHOW_TIME[0], SHOW_TIME[1], SHOW_TIME[2], SHOW_TIME[3],
            SHOW_TIME[4], SHOW_TIME[5]);
...
extern uint8_t back_point[109][2];
extern uint8_t num_point[28][2];
extern uint8_t fill_point[39][2];
...
void PIC_DISPLAY() {
    ...
    // drawcircle(40, 32, 26);
    // drawcircle(40, 32, 13);
    // uint8_t back_point[428][2]={

    {
        //绘制background
        for (int i = 0; i < 109; i++) {
            DrawPoint(40 + back_point[i][0] - 26, 32 + (back_point[i][1] - 26)); //右下
            DrawPoint(40 - (back_point[i][0] - 26), 32 + (back_point[i][1] - 26)); //左下
            DrawPoint(40 + back_point[i][0] - 26, 32 - (back_point[i][1] - 26)); //右上
            DrawPoint(40 - (back_point[i][0] - 26), 32 - (back_point[i][1] - 26)); //左上
        }
        uint8_t start_x = 40 - 39;
        uint8_t start_y = 32;
        uint8_t r = 39;
        for (int i = 0; i <= r; i++) {
            DrawPoint(start_x + i + r, start_y);
    ...
    for (int i = 0; i < 28; i++) //绘制卫星
        DrawPoint(num_point[i][0] - 5 + center_x, center_y + num_point[i][1] - 5);
...
void Write_LOCAL() {
    ...
    if (observer.height >= -999999 && observer.height <= 999999)
        EEPROM_WriteBuffer(
            0x2BB0 + 16, (uint8_t *) &observer.height, 8);
}
...
void SATE_KEY() {
    switch (my_kbd.current) {
        ...
        case KEY_MENU:
            if (menuState == (registerSpecsCount - 1U)) {
                menuState = 1;
            } else {
                menuState++;
            }
...
void TLE_KEY() {
    if (!TLE_KeyEventShouldFire(my_kbd.current)) {
        return;
    }
...
void TLE_PROCESS() {
    ...
    if (mode == SATE
        ...
        if (DOPPLER_FLASH) {
            DOPPLER_FLASH = 0;
            uint8_t utc_time[6];
            memcpy(utc_time, my_time, sizeof(utc_time));
#if defined(ARDUINO_ARCH_ESP32)
            // Satellite propagation expects UTC; my_time is Beijing local time.
            beijingToUtcTime(utc_time);
#endif
            cal_sat(sat_get.name, sat_get.line1, sat_get.line2, observer, &look, &sub_point, utc_time);
            if (sat_get.TX_FREQ > 1800000)
                sat_get.UP_LINK = doppler_shift(sat_get.TX_FREQ, sub_point.sate_eci, sub_point.r_dot, look.observer_eci,
                                                10);

            sat_get.DOWN_LINK = doppler_shift(sat_get.RX_FREQ, sub_point.sate_eci, sub_point.r_dot, look.observer_eci,
                                              0);
...
// 比较两个时间，返回值：
// -1 表示 time2 更新，1 表示 time1 更新，0 表示相同
int compare_times(uint8_t time1[6], uint8_t time2[6]) {
    // 循环比较每个字段（年、月、日、时、分、秒）3 11 11      3 15 2
    for (int i = 0; i < 6; i++) {
        if (time1[i] < time2[i]) return 0; // time1 比 time2 更新
        if (time1[i] > time2[i]) return 1; // time2 比 time1 更新
    }
    // 如果循环完没有返回，说明时间相同
    return 0;
}
...
#ifdef ENABLE_RTC
    // Legacy DS3231 support (if enabled).
    DS3231_GetTime(my_time);
#else
    // ESP32: use the existing PCF8563 RTC (configured from the main menu).
    RTC_INIT();
    RTC_Get();
#endif
...
#ifndef ENABLE_RTC
    uint8_t prev_time[6];
    memcpy(prev_time, my_time, sizeof(prev_time));
#endif
    if (tle_check() == 0) {
...
#else
        RTC_Get();
        if (memcmp(prev_time, my_time, sizeof(prev_time)) != 0) {
            DOPPLER_FLASH = 1;
            memcpy(prev_time, my_time, sizeof(prev_time));
        }
#endif
...
void DS3231Handler(void) {
    uint8_t get_time[6];
    DS3231_GetTime(get_time);
    for (uint8_t i = 0; i < 6; i++) {
        if (get_time[i] != my_time[i]) {
            DOPPLER_FLASH = 1;
            for (uint8_t j = 0; j < 6; j++) my_time[j] = get_time[j];
            return;
        }
    }
}
