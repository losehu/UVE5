...
_Static_assert(ARRAY_SIZE(ProcessKeysFunctions) == DISPLAY_N_ELEM, "ProcessKeysFunctions array size mismatch");
...
_Static_assert(ARRAY_SIZE(HandleFunction_fn_table) == FUNCTION_N_ELEM, "HandleFunction_fn_table array size mismatch");
...
// -------------------- PTT ------------------------
    ...
    } else if (digitalRead(GPIO_PTT) == LOW && !SerialConfigInProgress()) {    // PTT pressed
...
static void ProcessKey(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
    ...
    if (gIgnoreArduboySide2Release &&
        Key == KEY_SIDE2 &&
        !bKeyPressed &&
        !bKeyHeld) {
        gIgnoreArduboySide2Release = false;
        return;
    }
#endif
