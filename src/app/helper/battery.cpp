...
_Static_assert(ARRAY_SIZE(Voltage2PercentageTable[BATTERY_TYPE_1600_MAH]) ==
              ARRAY_SIZE(Voltage2PercentageTable[BATTERY_TYPE_2200_MAH]),"Voltage2PercentageTable error");
...
void BATTERY_GetReadings(const bool bDisplayBatteryLevel) {
    ...
    if (gBatteryVoltageAverage > 890)
        ...
        /* Prevent out-of-bounds access when i == 1 (i-2 would underflow) */
        for (int i = 6; i >= 2; i--) {
            if (perc > levels[i - 2]) {
