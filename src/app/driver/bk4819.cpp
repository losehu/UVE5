...
// ===== ESP32 优化宏 =====
// 使用 Arduino GPIO 操作以兼容性
#define GPIO_SET_HIGH(pin)   digitalWrite(pin, HIGH)
#define GPIO_SET_LOW(pin)    digitalWrite(pin, LOW)
#define GPIO_READ(pin)       digitalRead(pin)

// 高分辨率延迟
static inline void esp32_delay_us(uint32_t us) {
    ets_delay_us(us);
}

// ===== 寄存器地址转换宏 =====
// 为了避免类型转换错误，创建转换宏
#define BK4819_REG_HEX(x) ((BK4819_REGISTER_t)(x))
...
void BK4819_Init(void) {
    ...
    gpio_conf.pin_bit_mask = (1ULL << GPIOC_PIN_BK4819_SCN) | 
                              (1ULL << GPIOC_PIN_BK4819_SCL) | 
                              (1ULL << GPIOC_PIN_BK4819_SDA);
    ...
    GPIO_SET_HIGH(GPIOC_PIN_BK4819_SCL);
    GPIO_SET_HIGH(GPIOC_PIN_BK4819_SDA);
...
static uint16_t BK4819_ReadU16(void) {
    ...
    // 配置 SDA 为输入
    gpio_set_direction(GPIOC_PIN_BK4819_SDA, GPIO_MODE_INPUT);
    esp32_delay_us(1);
    
    Value = 0;
    for (i = 0; i < 16; i++) {
        Value <<= 1;
        Value |= GPIO_READ(GPIOC_PIN_BK4819_SDA);
        
        GPIO_SET_HIGH(GPIOC_PIN_BK4819_SCL);
        esp32_delay_us(1);
        GPIO_SET_LOW(GPIOC_PIN_BK4819_SCL);
        esp32_delay_us(1);
    }
    
    // 配置 SDA 为输出
    gpio_set_direction(GPIOC_PIN_BK4819_SDA, GPIO_MODE_OUTPUT);
...
uint16_t BK4819_ReadRegister(BK4819_REGISTER_t Register) {
    ...
    GPIO_SET_LOW(GPIOC_PIN_BK4819_SCN);
...
void BK4819_WriteU8(uint8_t Data) {
    ...
    for (i = 0; i < 8; i++) {
        if ((Data & 0x80) == 0)
            GPIO_SET_LOW(GPIOC_PIN_BK4819_SDA);
        else
            GPIO_SET_HIGH(GPIOC_PIN_BK4819_SDA);
...
int8_t BK4819_GetRxGain_dB(void) {
    ...
    BK4819_REGISTER_t gainAddr = reg7e.gainIdx < 0 ? BK4819_REG_14 : (BK4819_REGISTER_t)(BK4819_REG_10 + reg7e.gainIdx);
...
void BK4819_SetRegValue(RegisterSpec s, uint16_t v) {
    uint16_t reg = BK4819_ReadRegister((BK4819_REGISTER_t)s.num);
    reg &= ~(s.mask << s.offset);
    BK4819_WriteRegister((BK4819_REGISTER_t)s.num, reg | (v << s.offset));
}
...
void BK4819_start_tone(const uint16_t frequency, const unsigned int level, const bool tx, const bool tx_mute)
{
    ...
    // mute TX
    BK4819_WriteRegister(BK4819_REG_HEX(0x50), (1u << 15) | 0x3B20);

    BK4819_WriteRegister(BK4819_REG_HEX(0x70), BK4819_REG_70_ENABLE_TONE1 | ((level & 0x7f) << BK4819_REG_70_SHIFT_TONE1_TUNING_GAIN));

    BK4819_WriteRegister(BK4819_REG_HEX(0x30), 0);

    if (!tx)
    {
        BK4819_WriteRegister(BK4819_REG_HEX(0x30),
//			BK4819_REG_30_ENABLE_VCO_CALIB |
//			BK4819_REG_30_ENABLE_UNKNOWN   |
//			BK4819_REG_30_ENABLE_RX_LINK   |
            BK4819_REG_30_ENABLE_AF_DAC    |
...
//			BK4819_REG_30_ENABLE_RX_DSP    |
    ...
    BK4819_WriteRegister(BK4819_REG_HEX(0x71), scale_freq(frequency));

    SYSTEM_DelayMs(1);

//	BK4819_SetAF(tx ? BK4819_AF_BEEP : BK4819_AF_ALAM);
    BK4819_SetAF(BK4819_AF_ALAM);  // RX
//	BK4819_SetAF(BK4819_AF_BEEP);  // TX

    if (!tx_mute)
        BK4819_WriteRegister(BK4819_REG_HEX(0x50), 0x3B20);   // 0011 1011 0010 0000
...
//	BK4819_EnterTxMute();
    ...
    BK4819_WriteRegister(BK4819_REG_HEX(0x70), 0);
...
//		SYSTEM_DelayMs(2);
        ...
        const uint16_t css_val = BK4819_ReadRegister(BK4819_REG_HEX(0x51));
        BK4819_WriteRegister(BK4819_REG_HEX(0x51), 0);

        // set the FM deviation level
        const uint16_t dev_val = BK4819_ReadRegister(BK4819_REG_HEX(0x40));
...
//			UART_printf("tx dev %04X\r\n", dev_val);
#endif
        {
            ...
            //BK4819_WriteRegister(BK4819_REG_HEX(0x40), (3u << 12) | (deviation & 0xfff));
            BK4819_WriteRegister(BK4819_REG_HEX(0x40), (dev_val & 0xf000) | (deviation & 0xfff));
        }
        ...
        //
        const uint16_t filt_val = BK4819_ReadRegister(BK4819_REG_HEX(0x2B));
        BK4819_WriteRegister(BK4819_REG_HEX(0x2B), (1u << 2) | (1u << 0));
        ...
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x58), // 0x37C3);   // 001 101 11 11 00 001 1
        ...
        // tone-2 = 1200Hz
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x72), scale_freq(1200));
        ...
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x70),   // 0 0000000 1 1100000
            ( 0u << 15) |    // 0
            ( 0u <<  8) |    // 0
            ( 1u <<  7) |    // 1
            (96u <<  0));    // 96
//			(127u <<  0));
        ...
        BK4819_WriteRegister(BK4819_REG_HEX(0x5D), ((size - 1) << 8));
        ...
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x5A), 0x0000);                   // bytes 1 & 2
        ...
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x5B), 0x0000);                   // bytes 2 & 3
        ...
        //
        BK4819_WriteRegister(BK4819_REG_HEX(0x5C), 0x5625);   // 010101100 0 100101
//		BK4819_WriteRegister(BK4819_REG_HEX(0x5C), 0xAA30);   // 101010100 0 110000
//		BK4819_WriteRegister(BK4819_REG_HEX(0x5C), 0x0030);   // 000000000 0 110000

        BK4819_WriteRegister(BK4819_REG_HEX(0x59), (1u << 15) | (1u << 14) | fsk_reg59);   // clear FIFO's
        BK4819_WriteRegister(BK4819_REG_HEX(0x59), fsk_reg59);                             // release the FIFO reset

        {	// load the entire packet data into the TX FIFO buffer
            unsigned int i;
            const uint16_t *p = (const uint16_t *)packet;
            for (i = 0; i < (size / sizeof(p[0])); i++)
                BK4819_WriteRegister(BK4819_REG_HEX(0x5F), p[i]);  // load 16-bits at a time
        }

        // enable tx interrupt
        BK4819_WriteRegister(BK4819_REG_HEX(0x3F), BK4819_REG_3F_FSK_TX_FINISHED);

        // enable FSK TX
        BK4819_WriteRegister(BK4819_REG_HEX(0x59), (1u << 11) | fsk_reg59);
        ...
        {	// packet time is ..
            ...
            {
                SYSTEM_DelayMs(4);
                if (BK4819_ReadRegister(BK4819_REG_HEX(0x0C)) & (1u << 0))
                {	// we have interrupt flags
                    BK4819_WriteRegister(BK4819_REG_HEX(0x02), 0);
                    if (BK4819_ReadRegister(BK4819_REG_HEX(0x02)) & BK4819_REG_02_FSK_TX_FINISHED)
        ...
        // disable FSK
        BK4819_WriteRegister(BK4819_REG_HEX(0x59), fsk_reg59);

        BK4819_WriteRegister(BK4819_REG_HEX(0x3F), 0);   // disable interrupts
        BK4819_WriteRegister(BK4819_REG_HEX(0x70), 0);
        BK4819_WriteRegister(BK4819_REG_HEX(0x58), 0);

        // restore FM deviation level
        BK4819_WriteRegister(BK4819_REG_HEX(0x40), dev_val);

        // restore TX/RX filtering
        BK4819_WriteRegister(BK4819_REG_HEX(0x2B), filt_val);

        // restore the CTCSS/CDCSS setting
        BK4819_WriteRegister(BK4819_REG_HEX(0x51), css_val);

        //BK4819_EnterTxMute();
        BK4819_WriteRegister(BK4819_REG_HEX(0x50), 0xBB20); // 1011 1011 0010 0000

        //BK4819_SetAF(BK4819_AF_MUTE);
        BK4819_WriteRegister(BK4819_REG_HEX(0x47), (1u << 14) | (1u << 13) | (BK4819_AF_MUTE << 8) | (1u << 6));
...
void enable_msg_rx(const bool enable) {
    ...
    if (enable) {
        ...
        BK4819_WriteRegister(BK4819_REG_HEX(0x70),
                             (0u << 15) |    // 0
                             (0u << 8) |    // 0
                             (1u << 7) |    // 1
                             (96u << 0));    // 96
        ...
        BK4819_WriteRegister(BK4819_REG_HEX(0x58),
                             (1u << 13) |        // 1 FSK TX mode selection
                             //   0 = FSK 1.2K and FSK 2.4K TX .. no tones, direct FM
                             //   1 = FFSK 1200 / 1800 TX
                             //   2 = ???
        ...
        // <15:8> sync byte 0
        // < 7:0> sync byte 1
//			BK4819_WriteRegister(BK4819_REG_HEX(0x5A), ((uint16_t)mdc1200_sync_suc_xor[0] << 8) | (mdc1200_sync_suc_xor[1] << 0));
        BK4819_WriteRegister(BK4819_REG_HEX(0x5A), 0x7240); //0x7240

        // REG_5B .. bytes 2 & 3 sync pattern
        //
        // <15:8> sync byte 2
        // < 7:0> sync byte 3
//			BK4819_WriteRegister(BK4819_REG_HEX(0x5B), ((uint16_t)mdc1200_sync_suc_xor[2] << 8) | (mdc1200_sync_suc_xor[3] << 0));
        BK4819_WriteRegister(BK4819_REG_HEX(0x5B), 0x99a7);//0x99a7

        // disable CRC
        BK4819_WriteRegister(BK4819_REG_HEX(0x5C), 0x5625);   // 01010110 0 0 100101
//			BK4819_WriteRegister(BK4819_REG_HEX(0x5C), 0xAA30);   // 10101010 0 0 110000

        // set the almost full threshold
        BK4819_WriteRegister(BK4819_REG_HEX(0x5E), (64u << 3) | (1u << 0));  // 0 ~ 127, 0 ~ 7
...
//			}
        {    // packet size .. sync + 14 bytes - size of a single packet
            ...
            BK4819_WriteRegister(BK4819_REG_HEX(0x5D), (size << 8));
        }

        // clear FIFO's then enable RX
        BK4819_WriteRegister(BK4819_REG_HEX(0x59), (1u << 15) | (1u << 14) | fsk_reg59);
        BK4819_WriteRegister(BK4819_REG_HEX(0x59), (1u << 12) | fsk_reg59);
...
//			BK4819_RX_TurnOn();
        ...
        // enable interrupts
//			BK4819_WriteRegister(BK4819_REG_HEX(0x3F), BK4819_ReadRegister(BK4819_REG_HEX(0x3F)) | BK4819_REG_3F_FSK_RX_SYNC | BK4819_REG_3F_FSK_RX_FINISHED | BK4819_REG_3F_FSK_FIFO_ALMOST_FULL);
    } else {
        BK4819_WriteRegister(BK4819_REG_HEX(0x70), 0);
        BK4819_WriteRegister(BK4819_REG_HEX(0x58), 0);
    }
