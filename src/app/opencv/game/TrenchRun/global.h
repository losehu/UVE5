...
// When integrated into UVE5, reuse the global Arduboy2 shim instance.
#ifndef UVE5_USE_GLOBAL_ARDUBOY
Arduboy2 ab;
#else
#define ab ::arduboy
#endif
...
byte gameState = STATE_INTRO_CRAWL;

// Intro timing helpers (ms). Stored as globals so input-driven state changes
// can reset them reliably.
uint32_t introTextStartMs = 0;
uint32_t introCrawlLastStepMs = 0;
